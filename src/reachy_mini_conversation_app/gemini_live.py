...
class GeminiLiveHandler(ConversationHandler):
    ...
    def __init__(
        ...
        # v0.9.0 ConversationHandler.__init__ is no-arg; each backend now carries
        # its native sample rate per output frame as (rate, samples) rather than
        # as a base attribute. This handler emits (GEMINI_OUTPUT_SAMPLE_RATE, array)
        # directly and resamples input against GEMINI_INPUT_SAMPLE_RATE, so the old
        # rate kwargs are vestigial here.
        super().__init__()
